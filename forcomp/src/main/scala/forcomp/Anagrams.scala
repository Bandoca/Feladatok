...
object Anagrams {
  ...
  def sentenceOccurrences(s: Sentence): Occurrences = {
    val sentenceConcat: String = s.mkString("")
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    val masreszt = for {
      i <- x
      if y.forall(p => p._1 != i._1)
    } yield i
    (egyreszt ::: masreszt).sorted
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    ...
    def sentenceAnagramsSeged (occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) List(List())
      else
        for {
          sub <- combinations(occurrences)
          wordsFromDict <- dictionaryByOccurrences.getOrElse(sub,List())
          maradek <- sentenceAnagramsSeged(subtract(occurrences,sub))
        }
        yield wordsFromDict :: maradek
    }
sentenceAnagramsSeged(occ)
  }
}
